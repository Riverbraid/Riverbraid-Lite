...
void rb_persistence_save(const rb_cluster_state_t *state) {
    rb_cluster_state_t state_to_save = *state;
    pico_unique_board_id_t board_id;
    pico_get_unique_board_id(&board_id);

    hmac_sha256_init(board_id.id, PICO_UNIQUE_BOARD_ID_SIZE_BYTES);
    hmac_sha256_update((const uint8_t*)&state_to_save, sizeof(rb_cluster_state_t) - 32);
    hmac_sha256_final(state_to_save.hmac_sig);

    uint8_t buffer[FLASH_PAGE_SIZE];
    memset(buffer, 0, FLASH_PAGE_SIZE);
    memcpy(buffer, &state_to_save, sizeof(rb_cluster_state_t));
...
bool rb_persistence_load(rb_cluster_state_t *state) {
    ...
    memcpy(state, flash_target_contents, sizeof(rb_cluster_state_t));
    return true;
}
